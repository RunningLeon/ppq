...
class TensorRTQuantizer(BaseQuantizer):
    ...
    def __init__(self,
                 ...
                 per_tensor=True) -> Union[torch.Tensor, list, dict]:
        super().__init__(graph=graph)
        self._num_of_bits = 8
        self._quant_min = -128
        self._quant_max = +127
        self._weight_quant_policy = QuantizationProperty.PER_TENSOR if per_tensor else QuantizationProperty.PER_CHANNEL
        ...
        if operation.type in {'Conv', 'ConvTranspose', 'Gemm', 'MatMul'}:
            ...
            if operation.type in {'Conv', 'ConvTranspose'}:
                if operation.inputs[1].is_parameter:
                    conv_weight_config = base_quant_config.input_quantization_config[
                        1]
                    conv_weight_config.policy = QuantizationPolicy(
                        QuantizationProperty.SYMMETRICAL +
                        QuantizationProperty.LINEAR +
                        self._weight_quant_policy)
            ...
            if operation.num_of_input > 2:
                ...
                bias_config.policy = QuantizationPolicy(
                    QuantizationProperty.SYMMETRICAL +
                    QuantizationProperty.LINEAR + self._weight_quant_policy)
                bias_config.num_of_bits = 32
                bias_config.quant_max = int(pow(
                    2, bias_config.num_of_bits - 1)) - 1
                bias_config.quant_min = -int(
                    pow(2, bias_config.num_of_bits - 1)) + 1
    ...
    def quant_operation_types(self) -> set:
        return {
            ...
            'Resize', 'MaxPool', 'AveragePool', 'GlobalMaxPool',
            'GlobalAveragePool', 'Softmax', 'Mul', 'Add', 'Max', 'Sub', 'Div',
            'Reshape', 'LeakyRelu', 'Concat', 'Sigmoid', 'Interp',
            'ReduceMean', 'Transpose', 'Slice', 'Flatten'
        }

    @property
    def quantize_policy(self) -> QuantizationPolicy:
        return QuantizationPolicy(QuantizationProperty.SYMMETRICAL +
                                  QuantizationProperty.LINEAR +
